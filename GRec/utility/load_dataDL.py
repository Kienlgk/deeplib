...
class Data(object):
    ...
    def load_app_tpl_emb(self):
        tpl_emb_file_name = "/app/codebert/encoded_tpls.h5"
        with h5py.File(tpl_emb_file_name, "r") as fp:
            tpl_embs = np.asarray(fp["encoded_tpls"])
            app_embs = np.asarray(fp["encoded_apps"])
            # tpl_embs = fp.get('encoded_tpls').value
            # app_embs = fp.get('encoded_apps').value

            
        return app_embs, tpl_embs
